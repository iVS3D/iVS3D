...
QWidget* GeoDistance::getSettingsWidget(QWidget* parent) {
    ...
    if (!mpSamplingWidget) createSettingsWidget(parent);
    mpAltitudeCheckBox->setVisible(mAltitudeExisting);
    return mpSamplingWidget;
}

//==================================================================================================
QString GeoDistance::getName() const { return tr("GeoDistance"); }
...
std::vector<uint> GeoDistance::sampleImages(const std::vector<uint>& imageList,
    ...
    for (int i = 1; i < imageList.size(); i++) {
        ...
        if (imageList[i] != i) {
            for (int k = imageList[i - 1] + 1; k < imageList[i]; k++) {
                // mGpsData has to be filled with all gps values. Values that
                // aren't in the imageList are added here as false -> unused
                // values
                mGpsData.replace(
                    k, QPair<QPointF, bool>(mGpsData.at(k).first, false));
            }
        }
        currentDistance += distanceBetweenPoints(prevIndex, currentIndex);

        prevIndex = currentIndex;
        // if total distance between images is lower then the selected deviation
    ...
    if (imageList.back() != mpReader->getImageCount()) {
        for (int i = imageList.back(); i < mpReader->getImageCount(); i++) {
            // mGpsData has to be filled with all gps values. Values that aren't
            // in the imageList are added here
...
//==================================================================================================
void GeoDistance::initialize(iReader* reader, QMap<QString, QVariant> buffer,
                             signalObject* sigObj) {
...
//==================================================================================================
QMap<QString, QVariant> GeoDistance::generateSettings(Progressable* receiver,
                                                      bool useCuda,
                                                      volatile bool* stopped) {
...
void GeoDistance::createSettingsWidget(QWidget* parent) {
    ...
    QLabel* LabelDescription = new QLabel(
        tr("This plugin uses the geo location provided in the meta data to "
           "calculate the distance between images. An image is selected as a "
           "new keyframe, if the distance to the previous keyframe is greater "
    ...
    spinBoxWidget->layout()->addWidget(
        new QLabel(tr("Select distance in meter")));
    ...
    spinBoxWidget->setToolTip(
        tr("The minimum distance between two consecutive keyframes."));
    QObject::connect(mpSpinBoxDist,
                     QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
                     &GeoDistance::slot_distChanged);
    ...
    QObject::connect(mpAltitudeCheckBox, &QCheckBox::clicked, this,
                     &GeoDistance::slot_altitudeCheckChanged);
...
void GeoDistance::slot_distChanged(double n) { mDistance = n; }
