 ...
 * performed in sampleImages. Images are accessible by the Reader object. The currently selected images are passed as indices in imageList. This allows the plugin
...
public:
     ...
     * can be very time consuming, it is recommended to report progress to the given Progressable by invoking Progressable::slot_makeProgress. It should be
     ...
     * data can be buffered to accelerate the next call of sampleImages. On some systems the gpu can be utilized for acceleration as well, but only if useCuda is
     ...
     * @endcode
     *
     * @param imageList Index list of images to compute, but indices in between can be used for computation (see Blur)
    ...
    /**
     * @brief initialize the the IAlgorithm and the settings widget with plausible values from the Reader.
