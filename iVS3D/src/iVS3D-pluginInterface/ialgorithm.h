 ...
 * @ingroup Plugin
 *
 * @brief The IAlgorithm interface is used to include plugin implementations for
 * different sampling algorithms. As such the plugins need to present a display
 * name and have the possibility to display a custom QWidget to the user. The
 * main task for a IAlgorithm plugin to select keyframes based on the given
 * images. This is performed in sampleImages. Images are accessible by the
 * Reader object. The currently selected images are passed as indices in
 * imageList. This allows the plugin to sample on the current image selection as
 * well as include intermediate images in the computation. The plugin can write
 * data to an QVariant in order to buffer them. Buffered data is managed by the
 * core application and is passed to the plugin on the next sampleImages call.
 *
...
class IVS3DPLUGININTERFACE_EXPORT IAlgorithm : public QObject {
   ...
   public:
     ...
     * imageList provides indices for the currently selected keyframes. This way
     * the current selection can be taken in account in the calculation. Since
     * the algorithm can be very time consuming, it is recommended to report
     * progress to the given Progressable by invoking
     * Progressable::slot_makeProgress. It should be possible to abort
     * calculation by setting the value of stopped to @a true. Therefore it is
     * necessary to repeatedly check its state. To speedup calculation, data can
     * be buffered to accelerate the next call of sampleImages. On some systems
     * the gpu can be utilized for acceleration as well, but only if useCuda is
     ...
     *           Qt::DirectConnection,
     *           Q_ARG(int, progress),
     *           Q_ARG(QString, "computed image " + QString::number(i) + " of "
     * + QString::number(imageList.size())));
     *
     ...
     * can be used for computation (see Blur)
     ...
     * @see Rotation, Blur
     */
    virtual std::vector<uint> sampleImages(const std::vector<uint>& imageList,
                                           Progressable* receiver,
                                           volatile bool* stopped, bool useCuda,
                                           LogFileParent* logFile) = 0;
     ...
     */
    virtual void initialize(iReader* reader, QMap<QString, QVariant> buffer,
                            signalObject* sigObj) = 0;
     ...
     * @return QMap with the settings
     */
    virtual QMap<QString, QVariant> generateSettings(Progressable* receiver,
                                                     bool useCuda,
   ...
   signals:
    ...
    /**
     * @brief updateBuffer stores the given buffer for future uses of the
     * buffered data.
