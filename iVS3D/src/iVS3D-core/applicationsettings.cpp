...
QLocale ApplicationSettings::getLocale() { return m_locale; }
...
QString ApplicationSettings::getDefaultPluginName() {
...
void ApplicationSettings::loadSettings() {
    ...
    m_standardInputPath =
        settings
            .value(stringContainer::standardInputPathIdentifier,
                   m_standardInputPath)
            .value<QString>();
    m_disableChecks =
        settings
            .value(stringContainer::disableChecksIdentifier, m_disableChecks)
            .value<bool>();
    m_colorTheme =
        settings.value(stringContainer::colorThemeIdentifier, m_colorTheme)
            .value<ColorTheme>();
    if (settings.contains(stringContainer::useCudaIdentifier))
        m_useCuda =
            settings.value(stringContainer::useCudaIdentifier).value<bool>();
    QVariantMap reconstructMap =
        settings.value(stringContainer::reconstructSoftwareIdentifier)
            .value<QVariantMap>();
    ...
    m_createLogs =
        settings.value(stringContainer::createLogsIdentifier).value<bool>();
    m_interpolateMetaData =
        settings.value(stringContainer::interpolateIdentifier).value<bool>();
    m_locale =
        settings.value(stringContainer::localeIdentifier).value<QLocale>();
    m_defaultPluginName = settings
                              .value(stringContainer::defaultPluginIdentifier,
                                     m_defaultPluginName)
                              .value<QString>();
...
void ApplicationSettings::saveSettings() {
    QSettings settings(stringContainer::settingsCompany,
                       stringContainer::settingsProgramm);
    settings.setValue(stringContainer::standardInputPathIdentifier,
                      m_standardInputPath);
    settings.setValue(stringContainer::disableChecksIdentifier,
                      m_disableChecks);
    ...
    settings.setValue(stringContainer::interpolateIdentifier,
                      m_interpolateMetaData);
    QVariantMap reconstructMap;
    ...
    settings.setValue(stringContainer::reconstructSoftwareIdentifier,
                      reconstructMap);
    ...
    settings.setValue(stringContainer::defaultPluginIdentifier,
                      m_defaultPluginName);
    settings.setValue(stringContainer::forceBackupVideoReader,
                      m_forceBackupVideoReader);
}
...
bool ApplicationSettings::getDisableChecks() { return m_disableChecks; }

ColorTheme ApplicationSettings::getColorTheme() { return m_colorTheme; }
...
bool ApplicationSettings::getCreateLogs() { return m_createLogs; }

bool ApplicationSettings::get_forceBackupVideoReader() {
    return m_forceBackupVideoReader;
}
