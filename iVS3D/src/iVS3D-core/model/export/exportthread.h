...
struct ExportResult {
    ...
    static ExportResult success() { return {ExportResultType::Success, 0, ""}; }
    static ExportResult partialSuccess(int broken) {
        return {ExportResultType::PartialSuccess, broken, ""};
    }
    static ExportResult aborted() { return {ExportResultType::Aborted, 0, ""}; }
 ...
 * @ingroup Model
 *
 * @brief The ExportThread class provides functionality to export images with
 * given resolution and region of interest to a specific location. It extends
 * QThread in order to be executed in a separate thread.
 *
 * @author Dominik Wüst
 *
 * @date 2021/04/14
...
class ExportThread : public QThread {
   ...
   public:
    /**
     * @brief ExportThread creates an instance which reports progress and
     * exports images from given ModelInputPictures. Start using
     * QThread::start() to execute in separate thread!
     ...
     * @param stopped Thread continues export if @a false, aborts export if @a
     * false
     * @param roi The region of interest
     * @param iTransformCopies The ITransform instances to run fro creating
     * additional images
     */
   ...
   protected:
    /**
     * @brief run implements the export logic and is executed in a worker
     * thread.
     */
    void run() override;
};
