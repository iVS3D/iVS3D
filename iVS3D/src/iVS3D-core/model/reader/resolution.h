...
/**
 * @brief The Resolution class encapsulates an image resolution (width and
 * height). It provides functionality for parsing from a string, converting from
 * and to different Qt and OpenCV formats and for resizing images to the
...
class Resolution {
   ...
   public:
    /**
     * @brief Default constructor initializing width and height to zero. This
     * resolution is not valid, but can i.e. be initialized from a string!
     */
     ...
     * @param img OpenCV Mat where width and height are extracted from its
     * dimensions.
     */
    ...
    /**
     * @brief Checks whether the resolution is valid (i.e., width and height are
     * greater than zero).
     ...
     * @return True if both resolutions have the same width and height, false
     * otherwise.
     */
    ...
    /**
     * @brief Resizes an OpenCV Mat image to the resolution defined in this
     * object.
     * @param src Input image.
     * @param dest Output image (resized).
     */
    void resize(const cv::Mat& src, cv::Mat& dest) const {
        cv::resize(src, dest, this->toCvSize(), 0, 0, cv::INTER_AREA);
    }
