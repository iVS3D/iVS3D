...
std::vector<uint> AlgorithmManager::sample(std::vector<uint> sharpImages,
                                           ...
                                           volatile bool* stopped, int idx,
                                           bool useCuda,
                                           LogFileParent* logFile) {
    return m_algorithmList[idx]->sampleImages(sharpImages, receiver, stopped,
                                              useCuda, logFile);
}
...
void AlgorithmManager::initializePlugins(
    iReader* reader, QMap<QString, QMap<QString, QVariant>> allBuffer) {
...
QMap<QString, QVariant> AlgorithmManager::generateSettings(
    int idx, Progressable* receiver, bool useCuda, volatile bool* stopped) {
...
void AlgorithmManager::notifyNewMetaData() { m_sigObj->newMetaData(); }
...
void AlgorithmManager::loadPlugins() {
    ...
    for (const QString& fileName : entries) {
        if (fileName == "libiVS3D-pluginInterface.so") continue;
        ...
        if (plugin) {
            ...
            if (algorithm) {
                ...
                connect(algorithm, &IAlgorithm::updateKeyframes, this,
                        &AlgorithmManager::slot_updateKeyframes);
                connect(algorithm, &IAlgorithm::updateBuffer, this,
                        &AlgorithmManager::slot_updateBuffer);
