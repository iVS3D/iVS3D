...
MainWindow::MainWindow(QWidget* parent, ColorTheme theme, int cuda,
                       bool createLog, bool interpolateMetaData,
                       QList<QLocale> locales, QLocale selectedLocale,
                       bool forceBackupVideoReader, QWidget* otsWidget)
    : QMainWindow(parent), ui(new Ui::MainWindow) {
    ui->setupUi(this);

    // --- create GUI Elements VideoPlayer, Timeline and Info-Widgets with
    // Buttons
    ...
    m_videoplayer->addWidgetToLayout(
        m_timeline);  // timeline is part of videoplayer-layout
    ...
    ui->menuView->addAction(
        tr("Reset Layout"),
        [this, defaultDockLayout]() { this->restoreState(defaultDockLayout); },
    ...
    connect(m_inputWidget, &InfoWidget::sig_openFolderPressed, this,
            &MainWindow::on_actionOpen_Input_triggered);
    connect(m_inputWidget, &InfoWidget::sig_openVideoPressed, this,
            &MainWindow::on_actionOpen_Input_Video_triggered);
    connect(m_inputWidget, &InfoWidget::sig_openMetaPressed, this,
            &MainWindow::on_actionOpen_Meta_Data_triggered);

    // connect videoplayer widget
    connect(m_videoplayer, &VideoPlayer::sig_deleteAllKeyframes, this,
            &MainWindow::on_actionDelete_All_Keyframes_triggered);
    ...
    if (cuda < 0) {
        ...
        switch (cuda) {
            case -1:
                err_msg = tr("No GPU found");
                break;
            case -2:
                err_msg = tr("Compute Capability not supported");
                break;
            case -3:
                err_msg = tr("Built without CUDA");
                break;
            default:
    ...
    // set force to backup reader
    ui->actionForce_Backup_Video_Reader->setChecked(forceBackupVideoReader);
    connect(ui->actionForce_Backup_Video_Reader, &QAction::triggered, this,
            &MainWindow::on_actionForce_Backup_Video_Reader);
    ...
    ui->actionOpen_Input->setIcon(QIcon(
        theme == DARK ? ":/icons/openFolderIconW" : ":/icons/openFolderIconB"));
    ui->actionOpen_Input_Video->setIcon(QIcon(
        theme == DARK ? ":/icons/openVideoIconW" : ":/icons/openVideoIconB"));
    ui->actionOpen_Meta_Data->setIcon(QIcon(
        theme == DARK ? ":/icons/openMetaIconW" : ":/icons/openMetaIconB"));
...
void MainWindow::closeEvent(QCloseEvent* event) {
    QSettings settings(stringContainer::settingsCompany,
                       stringContainer::settingsProgramm);
...
void MainWindow::readSettings() {
    QSettings settings(stringContainer::settingsCompany,
                       stringContainer::settingsProgramm);
    if (settings.value("layoutVersion")
            .isNull()  // no version -> very old layout file, dont use!
        || settings.value("layoutVersion")
                   .value<QString>()
                   .compare(QString(UI_LAYOUT_VERSION)) !=
               0  // version of stored layout does not match current layout,
                  // dont use!
    ) {
...
VideoPlayer* MainWindow::getVideoPlayer() { return m_videoplayer; }

Timeline* MainWindow::getTimeline() { return m_timeline; }

OutputWidget* MainWindow::getOutputWidget() { return m_outputWidget; }

AutomaticWidget* MainWindow::getAutoWidget() { return m_autoWidget; }

InfoWidget* MainWindow::getInputWidget() { return m_inputWidget; }

SamplingWidget* MainWindow::getSamplingWidget() { return m_samplingWidget; }
...
void MainWindow::enableUndo(bool status) { ui->actionUndo->setEnabled(status); }

void MainWindow::enableRedo(bool status) { ui->actionRedo->setEnabled(status); }
...
void MainWindow::addOtsWindow(QWidget* otsWidget) {
    ...
    m_outputDock->raise();
    ui->menuView->insertAction(m_outputDock->toggleViewAction(),
...
bool MainWindow::getInputEnabled() { return ui->actionOpen_Input->isEnabled(); }

void MainWindow::setColorTheme(ColorTheme theme) {
    // assign a color palette for each theme
    QApplication::setStyle(theme == ColorTheme::DARK
                               ? new DarkStyle
    ...
    ui->actionRedo->setIcon(
        QIcon(theme == DARK ? ":/icons/redoIconW" : ":/icons/redoIconB"));
    ui->actionUndo->setIcon(
        QIcon(theme == DARK ? ":/icons/undoIconW" : ":/icons/undoIconB"));
    ui->actionDelete_All_Keyframes->setIcon(
        QIcon(theme == DARK ? ":/icons/resetIconW" : ":/icons/resetIconB"));
...
OperationStack* MainWindow::getOpStack() { return m_inputWidget->getOpStack(); }

void MainWindow::on_actionOpen_Project_triggered() { emit sig_openProject(); }

void MainWindow::on_actionSave_Project_triggered() { emit sig_saveProject(); }

void MainWindow::on_actionSave_Project_As_triggered() {
    emit sig_saveProjectAs();
}

void MainWindow::on_actionOpen_Input_triggered() { emit sig_openInputFolder(); }
...
std::vector<uint> MainWindow::generateKeyframes(uint totalFrames,
                                                uint keyframeCount) {
    ...
    while (keyframeCount != 0) {
        ...
        if (keyframes.empty() || std::find(keyframes.begin(), keyframes.end(),
                                           index) == keyframes.end()) {
...
void MainWindow::on_actionToggleTheme_triggered() { emit sig_toggleTheme(); }
...
void MainWindow::on_actionInterpolate_missing_meta_data_triggered() {
    emit sig_changeInterpolateMetaData(
        ui->actionInterpolate_missing_meta_data->isChecked());
}

void MainWindow::on_actionUndo_triggered() { emit sig_undo(); }

void MainWindow::on_actionRedo_triggered() { emit sig_redo(); }
...
void MainWindow::on_changeLanguage() {
    ...
    if (action && action->isChecked()) {
        ...
        QString msg =
            "Selected Language: " + selectedLocale.nativeLanguageName();
        slot_displayStatusMessage(msg);
        emit sig_selectLanguage(selectedLocale);

        // the language matches the currently active language -> no restart
        // required
        if (selectedLocale.language() ==
            qApp->property("translation").toLocale().language()) {
            return;
        }
        ...
        msgBox.setText(tr(
            "The application needs to be restarted for the language change to "
            "take effect. Make sure to save your project before restarting!"));
...
void MainWindow::on_actionForce_Backup_Video_Reader() {
    ...
    // Otherwise ask the user to restart the application
    QMessageBox msgBox(this);
    msgBox.setWindowTitle(tr("Force Backup Video Reader"));
    msgBox.setText(
        tr("The application needs to be restarted for a new reader to be "
           "loaded. Make sure to save your project before restarting!"));
