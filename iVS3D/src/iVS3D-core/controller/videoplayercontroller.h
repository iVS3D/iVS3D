 ...
 * @ingroup Controller
 *
 * @brief The VideoPlayerController class manages visualization of image data
 * and manual changes to keyframes.
 *
 * The image data is provided by the DataManager *dataManager. For visualization
 * a VideoPlayer *player and a Timeline *timeline are used. The user can select
 * images and modify keyframes using Timeline and VideoPlayer functions which
 * delegate the command to this VideoPlayerController using signals. This
 * VideoPlayerController is tasked with updating the data provided by
 * dataManager as well as refreshing the view-elements (Timeline and
 * VideoPlayer) periodically and after changes to the image data.
 *
 * @author Dominik Wüst
 *
 * @date 2021/02/10
...
class VideoPlayerController : public QObject {
   ...
   public:
     ...
     * Connects to the signals from VideoPlayer and Timeline and initializes
     * both with image data from DataManager. Initializes a timer to interrupt
     * periodically.
     ...
     * @param algoController an AlgorithmController instance to transform images
     * and preview them
     */
    explicit VideoPlayerController(
        QObject* parent = nullptr, VideoPlayer* player = nullptr,
        Timeline* timeline = nullptr, DataManager* dataManager = nullptr,
        AlgorithmController* algoController = nullptr);

    /**
     * @brief disconnects signals, deletes timer and VideoPlayerController
     * instance.
     */
    ~VideoPlayerController();

    /**
     * @brief getImageIndexOnScreen getter for m_imageIndexOnScreen (only used
     * in tests for now)
   ...
   public slots:
    /**
     * @brief [slot] slot_play() toggles automatic iteration over image
     * sequence.
     */
    void slot_play();
    ...
    /**
     * @brief [slot] slot_showPreviousImage() displays image going one or more
     * steps backward from current image.
     * @see VideoPlayerController::slot_changeStepSize to change stepsize.
     */
    void slot_showPreviousImage();

    /**
     * @brief [slot] slot_showNextImage() displays image going one or more steps
     * forward from current image.
     * @see VideoPlayerController::slot_changeStepSize to change stepsize.
     */
    void slot_showNextImage();

    /**
     * @brief [slot] slot_toggleKeyframe() toggles keyframe-state for currently
     * displayed image.
     */
    void slot_toggleKeyframe();

    /**
     * @brief [slot] slot_toggleKeyframesOnly(...) switches between displaying
     * all images or keyframes only.
     * @param checked display all images if @a false, keyframes only if @a true
     */
    void slot_toggleKeyframesOnly(bool checked);

    /**
     * @brief [slot] slot_changeStepSize(...) changes stepsize for iterating the
     * images.
    ...
    /**
     * @brief [slot] slot_changeIndex(...) displays the image from DataManager
     * referenced by given index.
     * @param index of image to display
     */
    void slot_changeIndex(unsigned int index);

    /**
     * @brief [slot] slot_mipChanged() is called when image data changed.
     * VideoPlayer and Timeline are updated.
     */
    ...
    /**
     * @brief [slot] slot_updateBoundaries() updates the mip so that the
     * boundaries are up to date
     */
    void slot_updateBoundaries(QPoint boundaries);

    /**
     * @brief [slot] slot_resetBoundaries() updates the mip so that the
     * boundaries are back to default
     */
    void slot_resetBoundaries();

    /**
     * @brief [slot] slot_imageProcessed(...) is invoked when image has been
     * processed and provides additional images.
    ...
    /**
     * @brief [slot] slot_receiveImage draws given image with and applies a
     * transformable plugin if neccessary
    ...
    /**
     * @brief [slot] slot_selectedITransformChanged connects new plugin to
     * videoplayer
     * @param id plugin id
     */
    ...
    /**
     * @brief slot_transformEnabledChanged reads data with new changed enabled
     * parameter
   ...
   signals:
    ...
    /**
     * @brief [signal] sig_hasStatusMessage(...) is emitted when
     * VideoPlayerController has a status message to display to the user.
     * @param message the QString to display.
     */
    void sig_hasStatusMessage(QString message);

    /**
     * @brief [signal] sig_imageToProcess(...) is emitted when VideoPlayer is
     * about to display a new image.
    ...
    /**
     * @brief [signal] sig_read sends an image request to the worker thread to
     * read the image.
    ...
    /**
     * @brief [signal] sig_sendToITransform sends the image for processing to
     * the connected ITransforms.
     * @param idx The image index
     * @param img The image to process
     */
    void sig_sendToITransform(uint idx, const cv::Mat& img,
                              const Resolution& resolution, const ROI& roi);

    /**
     * @brief [signal] sig_toggleKeyframe notifies about a manual keyframe
     * change by clicking Add keyframe/Remove keyframe
     * @param idx Index of the changed frame
     * @param isNowKeyframe @a true if the frame wasn't a keyframe and became
     * one by the user, @a false otherwise
     */
    void sig_toggleKeyframe(uint idx, bool isNowKeyframe);

    /**
     * @brief [signal] sig_deleteKeyframes is emitted when all keyframes are
     * deleted
     */
    void sig_deleteKeyframes();

    /**
     * @brief [signal] sig_deleteAllKeyframes is emitted when frames are reseted
     * to be keyframes
     */
