...
#include "reader/roi.h"

VideoPlayerController::VideoPlayerController(
    QObject *parent, VideoPlayer *player, Timeline *timeline,
    DataManager *dataManager, AlgorithmController *algoController)
    : QObject(parent) {
  ...
  ConcurrentReader *r =
      m_dataManager->getModelInputPictures()->createConcurrentReader();
  r->moveToThread(&workerThread);
  connect(&workerThread, &QThread::finished, r, &ConcurrentReader::deleteLater);
  connect(this, &VideoPlayerController::sig_read, r,
          &ConcurrentReader::slot_read);
  connect(r, &ConcurrentReader::sig_imageReady, this,
          &VideoPlayerController::slot_receiveImage);
  workerThread.start();

  m_transform = nullptr;
  TransformManager *tm = &TransformManager::instance();
  connect(tm, &TransformManager::sig_selectedTransformChanged, this,
          &VideoPlayerController::slot_selectedITransformChanged);
  connect(tm, &TransformManager::sig_transformEnabledChanged, this,
          &VideoPlayerController::slot_transformEnabledChanged);
...
void VideoPlayerController::slot_showPreviousImage() {
  m_imageIndex = m_iterator->getPrevious(m_dataManager->getModelInputPictures(),
...
void VideoPlayerController::slot_deleteAllKeyframes() {
  ...
  if (rdd.exec() == QDialog::Accepted) {
    ...
    m_dataManager->getModelInputPictures()->setBoundaries(
        QPoint(0, m_dataManager->getModelInputPictures()->getImageCount() - 1));
    std::vector<uint> allFrames;
    allFrames.reserve(m_dataManager->getModelInputPictures()->getImageCount());
    for (uint i = 0;
         i < m_dataManager->getModelInputPictures()->getImageCount(); i++) {
...
void VideoPlayerController::slot_updateBoundaries(QPoint boundaries) {
  int frameCount = m_dataManager->getModelInputPictures()->getImageCount();
  if (boundaries.x() < 0)
    boundaries.setX(0);
...
void VideoPlayerController::slot_redraw() { m_imageIndexOnScreen = UINT_MAX; }
  ...
  if (img.empty()) {
    ...
    for (int i : m_foundCorruptedFrames) {
      ...
      if (listedFramesCount <
          int(m_foundCorruptedFrames.size()) - 1) // prevent trailing ", "
    ...
    emit sig_hasStatusMessage(
        errorMsg.arg(QString::fromStdString(ss_foundCorruptedFrames.str())));
...
void VideoPlayerController::slot_selectedITransformChanged(uint id) {
  if (m_transform) {
    disconnect(m_transform, &ITransform::sendToGui, this,
               &VideoPlayerController::slot_displayImage);
    disconnect(this, &VideoPlayerController::sig_sendToITransform, m_transform,
               &ITransformRequestDequeue::slot_transform);
  }
  m_transform = TransformManager::instance().getTransform(id);
  if (m_transform) {
    connect(m_transform, &ITransform::sendToGui, this,
            &VideoPlayerController::slot_displayImage);
    connect(this, &VideoPlayerController::sig_sendToITransform, m_transform,
            &ITransformRequestDequeue::slot_transform);
  }
