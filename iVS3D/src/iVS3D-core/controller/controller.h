 ...
 * @ingroup Controller
 *
 * @brief The Controller class is the main controller and the starting point of
 * iVS3D. Its responsible for initializing the other controllers, the
 * DataManager, the MainWindow, AlgorithmManager and TransformManager. This
 * includes creating the instances and connecting their signal & slots. The
 * Controller also handels user input on the menu bar.
 *
 * If a new Import was successfully loaded Controller will delete all existing
 * controllers and creat new instances of them.
 *
...
class Controller : public QObject {
   ...
   public:
    /**
     * @brief Controllers constructor which creates AlgorithmManager,
     * TransformManager, DataManager and MainWindow and connecting existing
     * signals
     */
   ...
   public slots:
    /**
     * @brief slot_openInputFolder Slot is called when a folder should be
     * loaded. It will show a file dialog and use the DataManager to import the
     * selected folder.
     */
    void slot_openInputFolder();
    /**
     * @brief slot_openInputVideo Slot is called when a video should be loaded.
     * It will show a file dialog and use the OpenExecutor to import the
     * selected video.
     */
    void slot_openInputVideo();
    /**
     * @brief slot_openVideoDragAndDrop Slot is called when a file is drop on
     * the VideoPlayer. It will try to import the drop file with the
     * OpenExecutor.
     */
    ...
    /**
     * @brief slot_saveProjectAs Slot is called when a project should be saved
     * to a new location It will show a file dialog and use the DataManager to
    ...
    /**
     * @brief slot_addReconstructPath Slot is called when 'Manage reconstrution
     * tools' is clicked. It will execute a ReconstructionToolsDialog.
     */
    void slot_addReconstructPath();
    /**
     * @brief slot_changeDefaultInputPath Slot is called when 'Set input Path'
     * is clicked. It will show a file dialog and save the selected folder in
     * ApplicationSettings.
     */
    void slot_changeDefaultInputPath();
    /**
     * @brief slot_toggleColorTheme Slot is called when color theme is changed
     * by the user. Toggles between different themes.
     */
    void slot_toggleColorTheme();
    /**
     * @brief slot_changeColorTheme Slot is called when a specific color theme
     * is selected.
    ...
    /**
     * @brief slot_changeUseCuda Slot is called when 'create log file' is
     * clicked.
     * @param createLog @a true if create log file is marked
     */
    void slot_changeCreateLogFile(bool createLog);
    /**
     * @brief slot_changeInterpolateMetaData Slot is called when 'Interpolate
     * missing meta data' is clicked.
   ...
   signals:
    /**
     * @brief [signal] sig_hasStatusMessage() is emitted when the status message
     * is updated.
     *
    ...
    /**
     * @brief [signal] sig_colorThemeChanged() is emitted when the color theme
     * is changed.
