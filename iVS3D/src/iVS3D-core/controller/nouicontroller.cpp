...
noUIController::noUIController(QString inputPath, QString settingsPath,
...
int noUIController::exec() {
    ...
    if (m_autoPath.compare("") == 0 || m_inputPath.compare("") == 0 ||
        m_outputPath.compare("") == 0) {
        m_terminal->slot_displayMessage(
            tr("Auto settings file (-a), input (-i) and output (-o) need to be "
               "provided."));
    ...
    int numberImages = m_dataManager->getModelInputPictures()->getImageCount();
    // Check for valid input
    if (numberImages <= 0) {
        m_terminal->slot_displayMessage(
            tr("Opend no files. Please check your input path."));
        QCoreApplication::quit();
        return 0;
    }

    m_terminal->slot_displayMessage(
        tr("Opend ") + QString::number(numberImages) + tr(" images"));
    ...
    m_terminal->slot_displayMessage(
        tr("Loaded the following plugin settings:"));
    ...
    if (plugins.size() <= 0) {
        m_terminal->slot_displayMessage(
            tr("Imported no settings. Please check your settings file."));
        QCoreApplication::quit();
        return 0;
    }
    ...
    AlgorithmManager::instance().initializePlugins(
        ...
        m_dataManager->getModelAlgorithm()->getPluginBuffer());

    AutomaticExecutor* autoExec =
        new AutomaticExecutor(m_dataManager, autoSettings);
    ...
    m_terminal->slot_displayMessage(
        tr("Sampled ") + QString::number(keyframeCount) + tr(" keyframes."));

    m_terminal->slot_displayMessage(tr("\n### Finished Computation ###\n"));
    m_terminal->slot_displayMessage(
        tr("Finished in ") + QString::number(timer.elapsed() / 1000) + tr("s"));
