...
Controller::Controller(QString inputPath, QString settingsPath,
    ...
    m_mainWindow = new MainWindow(
        ...
        ApplicationSettings::instance().get_forceBackupVideoReader(),
        otsWidget);

    m_mainWindow->getSamplingWidget()->setPluginList(
        PluginManager::instance().getPluginNames());
...
void Controller::slot_exportStarted() { m_exporting = true; }
...
void Controller::slot_editCrop() {
    if (m_dataManager->getModelInputPictures()) {
        ...
        const cv::Mat img =
            m_dataManager->getModelInputPictures()->getReader()->getPic(
                current_idx, Reader::APPLY_NONE);
        if (img.empty()) {
...
void Controller::slot_restorePluginSettings(int id) {
    ...
    m_mainWindow->getSamplingWidget()->setResolution(
        record->workingResolution.toString());
    ...
    auto applySettingsResult = PluginManager::instance().applyPluginSettings(
        record->pluginName, record->pluginSettings);
    if (!applySettingsResult) {
        QMessageBox msgBox;
        msgBox.setText(tr("Error restoring plugin settings for plugin '") +
                       record->pluginName + tr("': ") +
                       applySettingsResult.error().message);
        msgBox.exec();
    }
    if (m_pluginController) {
        m_mainWindow->getSamplingWidget()->setSelectedPlugin(
            record->pluginName);
...
void Controller::setInputWidgetInfo() {
    ...
    if (currentReader->getFPS() != -1) {
        entries.append(
            {{tr("Video"), true},
             {QString::number(currentReader->getVideoDuration()) +
              tr(" seconds")},
...
void Controller::onSuccessfulOpen() {
    ...
    m_exportController =
        new ExportController(m_mainWindow->getOutputWidget(), m_dataManager,
                             m_colmapWrapper, m_pluginThread, m_stack);
    ...
    m_pluginController = new PluginController(
        ...
        m_videoPlayerController, m_stackController, m_pluginThread, m_stack);
    connect(m_mainWindow->getOutputWidget()->getMaskStackView().get(),
            &MaskStackView::sig_recordSelected, this,
            &Controller::slot_restorePluginSettings);
