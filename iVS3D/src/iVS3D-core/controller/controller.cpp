...
Controller::Controller(QString inputPath, QString settingsPath,
    ...
    QList<QLocale> locales =
        ApplicationSettings::instance().getAvailableLocales();
    QLocale selectedLocale = ApplicationSettings::instance().getLocale();

    QWidget* otsWidget = nullptr;
    const auto otsTheme =
        ApplicationSettings::instance().getColorTheme() == ColorTheme::DARK
            ? lib3d::ots::ui::ETheme::DARK
            : lib3d::ots::ui::ETheme::LIGHT;

    m_colmapWrapper->setChecksDisabled(
        ApplicationSettings::instance().getDisableChecks());
    ...
    otsWidget->layout()->addWidget(
        m_colmapWrapper->getOrCreateUiControlsFactory()->createViewWidget(
            nullptr));
    m_newProductPushButton =
        m_colmapWrapper->getOrCreateUiControlsFactory()
            ->createNewProductPushButton(otsTheme, nullptr);
    otsWidget->layout()->addWidget(m_newProductPushButton);

    bool interpolateMetaData =
        ApplicationSettings::instance().getInterpolateMetaData();
    m_mainWindow = new MainWindow(
        nullptr, ApplicationSettings::instance().getColorTheme(), useCuda,
        ApplicationSettings::instance().getCreateLogs(), interpolateMetaData,
        locales, selectedLocale, algorithms, transforms, otsWidget);

    m_mainWindow->enableUndo(false);
    m_mainWindow->enableRedo(false);
    m_mainWindow->enableTools(false);

    m_colmapWrapperSettingsAction =
        m_colmapWrapper->getOrCreateUiControlsFactory()->createSettingsAction(
            otsTheme, nullptr);
    ...
    if (AlgorithmManager::instance().getAlgorithmCount() +
            TransformManager::instance().getTransformCount() >
        0) {
        displayPluginSettings();
    }
    TransformManager::instance().enableCuda(
        ApplicationSettings::instance().getUseCuda());

    LogManager::instance().toggleLog(
    ...
    connect(m_mainWindow, &MainWindow::sig_openProject, this,
            &Controller::slot_openProject);
    connect(m_mainWindow, &MainWindow::sig_openInputFolder, this,
            &Controller::slot_openInputFolder);
    connect(m_mainWindow, &MainWindow::sig_openInputVideo, this,
            &Controller::slot_openInputVideo);
    connect(m_mainWindow, &MainWindow::sig_openVideoDragAndDrop, this,
            &Controller::slot_openDragAndDrop);
    connect(m_mainWindow, &MainWindow::sig_saveProjectAs, this,
            &Controller::slot_saveProjectAs);
    connect(m_mainWindow, &MainWindow::sig_saveProject, this,
            &Controller::slot_saveProject);
    connect(m_mainWindow, &MainWindow::sig_changeReconstructPath, this,
            &Controller::slot_addReconstructPath);
    connect(m_mainWindow, &MainWindow::sig_changeDefaultInputPath, this,
            &Controller::slot_changeDefaultInputPath);
    connect(m_mainWindow, &MainWindow::sig_toggleTheme, this,
            &Controller::slot_toggleColorTheme);
    connect(m_mainWindow, &MainWindow::sig_changeUseCuda, this,
            &Controller::slot_changeUseCuda);
    connect(m_mainWindow, &MainWindow::sig_changeCreateLogFile, this,
            &Controller::slot_changeCreateLogFile);
    connect(m_mainWindow, &MainWindow::sig_openMetaData, this,
            &Controller::slot_openMetaData);
    ...
    connect(m_mainWindow->getSamplingWidget(), &SamplingWidget::sig_resChanged,
            this, &Controller::slot_workingResolutionChanged);
    connect(m_mainWindow->getVideoPlayer(), &VideoPlayer::sig_cropEdit, this,
            &Controller::slot_editCrop);
    connect(m_mainWindow->getVideoPlayer(), &VideoPlayer::sig_useCropChanged,
            this, &Controller::slot_useCropChanged);
    connect(outputWidget, &OutputWidget::sig_altitudeChanged, this,
            &Controller::slot_altitudeChanged);

    connect(m_mainWindow, &MainWindow::sig_quit,
            m_colmapWrapper->getOrCreateUiControlsFactory(),
            &lib3d::ots::ui::ColmapWrapperControlsFactory::onQuit);
    connect(m_mainWindow, &MainWindow::sig_changeInterpolateMetaData, this,
            &Controller::slot_changeInterpolateMetaData);

    connect(this, &Controller::sig_hasStatusMessage, m_mainWindow,
            &MainWindow::slot_displayStatusMessage);

    m_automaticController = new AutomaticController(
        m_mainWindow->getOutputWidget(), m_mainWindow->getAutoWidget(),
        m_mainWindow->getSamplingWidget(), m_dataManager);
    ...
    m_mainWindow->getOutputWidget()->enableCreateFilesWidget(
        TransformManager::instance().getTransformCount() != 0);
...
void Controller::slot_openInputFolder() {
    if (m_exporting) {
        QMessageBox msgBox;
        msgBox.setText(
            tr("Wait for export to finish before importing new images."));
        msgBox.exec();
        return;
    }
    QString folderPath = QFileDialog::getExistingDirectory(
        m_mainWindow, tr("Choose Folder"),
...
void Controller::slot_openInputVideo() {
    if (m_exporting) {
        QMessageBox msgBox;
        msgBox.setText(
            tr("Wait for export to finish before importing a new video."));
        msgBox.exec();
        return;
    }
    QString selectedFilter = "";
    QString folderPath = QFileDialog::getOpenFileName(
        m_mainWindow, tr("Choose Video"),
        ApplicationSettings::instance().getStandardInputPath(),
        "*.mp4 *.mov *.avi", &selectedFilter, QFileDialog::DontUseNativeDialog);
    if (folderPath == nullptr) {
        emit sig_hasStatusMessage(tr("Input canceled"));
        return;
    }
...
void Controller::slot_openDragAndDrop(QString filePath) {
    ...
    if (m_exporting) {
        QMessageBox msgBox;
        msgBox.setText(
            tr("Wait for export to finish before importing new files."));
        msgBox.exec();
        return;
    }
    ...
    for (const QString& extension :
         MetaDataManager::supportedFileExtensions()) {
...
void Controller::slot_openProject() {
    if (m_exporting) {
        QMessageBox msgBox;
        msgBox.setText(
            tr("Wait for export to finish before importing new project."));
        msgBox.exec();
        return;
    }
    QString selectedFilter = "";
    QString folderPath = QFileDialog::getOpenFileName(
        m_mainWindow, tr("Choose project file"),
        ApplicationSettings::instance().getStandardInputPath(), "*.json",
...
void Controller::slot_saveProjectAs() {
    QString selectedFilter = "";
    QString projectPath = QFileDialog::QFileDialog::getSaveFileName(
        m_mainWindow, tr("Save project"),
        ApplicationSettings::instance().getStandardInputPath(), "*.json",
        &selectedFilter, QFileDialog::DontUseNativeDialog);
    ...
    if (0 != QString::compare(projectPath.split(".").last(), "json",
                              Qt::CaseSensitive)) {
        projectPath += ".json";
    }
    m_dataManager->saveProjectAs(getNameFromPath(projectPath, ".json"),
                                 projectPath);
...
void Controller::slot_changeDefaultInputPath() {
    QString folderPath = QFileDialog::getExistingDirectory(
        m_mainWindow, tr("Choose standard input path"),
        ApplicationSettings::instance().getStandardInputPath(),
        QFileDialog::DontUseNativeDialog);
    if (folderPath == nullptr) {
        emit sig_hasStatusMessage(tr("Input canceled"));
...
void Controller::slot_toggleColorTheme() {
    ...
    m_colmapWrapper->getOrCreateUiControlsFactory()->createSettingsAction(
        otsTheme, m_mainWindow, m_colmapWrapperSettingsAction);
    m_colmapWrapper->getOrCreateUiControlsFactory()->createNewProductPushButton(
        otsTheme, m_mainWindow, m_newProductPushButton);
...
void Controller::slot_changeColorTheme(ColorTheme theme) {
    ...
    emit sig_hasStatusMessage(
        tr("GUI color theme changed to ") +
        QString((theme == ColorTheme::DARK ? tr("dark") : tr("light"))) +
        tr(" style"));
}
...
void Controller::slot_changeUseCuda(bool useCuda) {
    ...
    emit sig_hasStatusMessage(useCuda ? tr("CUDA enabled")
                                      : tr("CUDA disabled"));
}

void Controller::slot_changeCreateLogFile(bool createLog) {
    ApplicationSettings::instance().setCreateLogs(createLog);
    QString msg = tr("Create log files") +
                  (QString)(createLog ? tr(" enabled") : tr(" disabled"));
...
void Controller::slot_changeInterpolateMetaData(bool interpolate) {
    ...
    QString msg = tr("Interpolating missing meta data") +
                  (QString)(interpolate ? tr(" enabled") : tr(" disabled"));
    emit sig_hasStatusMessage(msg);
...
void Controller::slot_openMetaData() {
    ...
    QString filePath = QFileDialog::getOpenFileName(
        m_mainWindow, tr("Choose Meta Data"),
        ApplicationSettings::instance().getStandardInputPath(),
        "*.srt *.SRT *.gpx *.GPX *.jpeg *.jpg *.txt", &selectedFilter,
        QFileDialog::DontUseNativeDialog);
    if (filePath == nullptr) {
        emit sig_hasStatusMessage(tr("Input canceled"));
        return;
...
void Controller::slot_openFinished(int result) {
    //    disconnect(m_openExec, &OpenExecutor::sig_finished, this,
    //    &Controller::slot_openFinished); disconnect(m_inputProgressDialog,
    //    &ProgressDialog::rejected, m_openExec, &OpenExecutor::slot_abort);
...
void Controller::slot_workingResolutionChanged(QString resolution) {
    ...
    bool valid = mip->setWorkingResolution(resolution);

    auto samplingWidget = m_mainWindow->getSamplingWidget();
    if (samplingWidget) return;
    samplingWidget->setResolutionValid(valid);
    if (!valid) m_videoPlayerController->slot_mipChanged();
}
...
void Controller::slot_editCrop() {
    ...
    if (m_dataManager->getModelInputPictures()) {
        ...
        const cv::Mat* img =
            m_dataManager->getModelInputPictures()->getPic(current_idx);
        if (img->empty()) {
            QMessageBox* em = new QMessageBox();
            em->setText(
                tr("The selected frame is broken and can´t be cropped. Please "
                   "select another frame to select a new region of intrest."));
            em->show();
            return;
        }
        ...
        if (dialog.exec() == QDialog::Accepted) {
            ...
            if (m_exportController) {
                m_exportController->slot_roiChanged(
                    params->getUseRoi() ? std::optional<ROI>(params->getRoi())
                                        : std::nullopt);
            }
        }
    }
...
void Controller::createOpenMessage(int numPics) {
    ...
    if (numPics <= 0) {
        emit sig_hasStatusMessage(tr("No images imported after ") +
        ...
        if (metaDataCount != 0) {
            emit sig_hasStatusMessage(
                tr("Import of ") + QString::number(numPics) +
                tr(" images and ") + QString::number(metaDataCount) +
                tr(" meta data feature") +
                QString(metaDataCount > 1 ? tr("s") : "") +
                tr(" finished after ") + QString::number(duration_ms) +
                tr("ms"));
        } else {
            emit sig_hasStatusMessage(tr("Import of ") +
                                      QString::number(numPics) +
                                      tr(" images finished after ") +
                                      QString::number(duration_ms) + tr("ms"));
        }
        onSuccessfulOpen();
    }
...
void Controller::setInputWidgetInfo() {
    ...
    QList<VideoPlayer::OverlayEntry> entries = {
        {m_dataManager->getModelInputPictures()->getPath(), false,
        ...
        {QString::number(
         ...
         tr(" images")},  // image count
    };
    iReader* currentReader =
        m_dataManager->getModelInputPictures()->getReader();
    if (currentReader->getAvgFps() != -1) {
        entries.append(
            {{tr("Video"), true},
             {QString::number(currentReader->getDuration()) + tr(" seconds")},
             {QString::number(currentReader->getAvgFps()) + tr(" fps")}});
    }
    QStringList loadedMetaData =
        MetaDataManager::instance().availableMetaData();
    if (loadedMetaData.size() != 0) {
        entries.push_back({tr("Metadata"), true});
        for (int i = 0; i < loadedMetaData.size(); i++) {
            entries.push_back(
                {" #" + QString::number(i + 1) + " " + loadedMetaData.at(i)});
        }
    }
    ...
    int resolutionIdx = 0;
    if (!(readerParams->getOriginalResolution() ==
          readerParams->getWorkingResolution())) {
        ...
        resolutionIdx =
            (it != resList.end()) ? std::distance(resList.begin(), it) : -1;
    ...
    m_mainWindow->getSamplingWidget()->setResolutionList(resList,
                                                         resolutionIdx);
...
void Controller::displayPluginSettings() {
    ...
    if (AlgorithmManager::instance().getAlgorithmCount() > 0) {
        settingsW =
            AlgorithmManager::instance().getSettingsWidget(samplingW, 0);
    } else {
        settingsW =
            TransformManager::instance().getSettingsWidget(samplingW, 0);
    }
...
void Controller::onFailedOpen() {
    ...
    if (m_videoPlayerController) {
        disconnect(m_videoPlayerController,
                   &VideoPlayerController::sig_hasStatusMessage, m_mainWindow,
                   &MainWindow::slot_displayStatusMessage);
        delete m_videoPlayerController;
        m_videoPlayerController = nullptr;
    }
    if (m_algorithmController) {
        disconnect(m_algorithmController,
                   &AlgorithmController::sig_hasStatusMessage, m_mainWindow,
                   &MainWindow::slot_displayStatusMessage);
        delete m_algorithmController;
        m_algorithmController = nullptr;
    }
    if (m_exportController) {
        disconnect(m_exportController, &ExportController::sig_hasStatusMessage,
                   m_mainWindow, &MainWindow::slot_displayStatusMessage);
    ...
    QMessageBox msgBox;
    msgBox.setText(
        tr("Failed to load input data. Possible causes are:\n"
           "- The selected folder does not contain any supported image or "
           "video files.\n"
...
uint Controller::loadMetaDataFromPath(QString path) {
    int n =
        m_dataManager->getModelInputPictures()->loadMetaData(QStringList(path));
    if (n > 0) {
        AlgorithmManager::instance().notifyNewMetaData();
        // Update the info widget
        setInputWidgetInfo();
        QString msg = tr("Loaded ") + QString::number(n) +
                      tr(" meta data feature") + QString(n > 1 ? tr("s") : "");
        emit sig_hasStatusMessage(msg);
        return n;
    } else {
...
bool Controller::loadInputDataFromPath(QString path) {
    ...
    bool validDatasetPath = path.endsWith(".mp4", Qt::CaseInsensitive) ||
    ...
    m_inputProgressDialog->slot_displayProgress(
        -1, tr("Importing dataset and metadata."));

    connect(m_inputProgressDialog, &ProgressDialog::rejected, m_openExec,
            &OpenExecutor::slot_abort);
    connect(m_openExec, &OpenExecutor::sig_finished, this,
            &Controller::slot_openFinished);
    m_timer = QElapsedTimer();
    m_timer.start();
    m_openExec->open();
...
void Controller::setAltitude() {
    ...
    for (MetaDataReader* reader : readers) {
        if (reader->getName().startsWith("GPS")) {
            ...
            double altitude_abs =
                gpsHash.find("GPSAltitude").value().toDouble();
            double altitude =
                (gpsHash.find("GPSAltitudeRef").value().toString() == "0")
                    ? altitude_abs
                    : altitude_abs * -1;
...
void Controller::onSuccessfulOpen() {
    ...
    if (m_dataManager->isProjectLoaded()) {
        m_mainWindow->showProjectTitle(m_dataManager->getProjectPath());
        emit sig_hasStatusMessage(
            tr("Project ") + m_dataManager->getProjectName() + tr(" with ") +
            QString::number(
                m_dataManager->getModelInputPictures()->getImageCount()) +
            tr(" images loaded"));
    } else {
    ...
    iReader* currentReader =
        m_dataManager->getModelInputPictures()->getReader();
    AlgorithmManager::instance().initializePlugins(
        currentReader, m_dataManager->getModelAlgorithm()->getPluginBuffer());
    AlgorithmManager::instance().notifyKeyframesChanged(
        m_dataManager->getModelInputPictures()->getAllKeyframes(false));
    if (currentReader->getMetaData() != nullptr &&
        currentReader->getMetaData()->availableMetaData().size() > 0) {
        AlgorithmManager::instance().notifyNewMetaData();
    }
    ...
    // AlgorithmController manages input widget and algorithm used widgets and
    // delegates image sampling
    m_algorithmController = new AlgorithmController(
        m_dataManager, m_mainWindow->getSamplingWidget());
    connect(m_algorithmController, &AlgorithmController::sig_hasStatusMessage,
            m_mainWindow, &MainWindow::slot_displayStatusMessage);

    // VideoPlayerControler manages video player and timeline
    m_videoPlayerController = new VideoPlayerController(
        this, m_mainWindow->getVideoPlayer(), m_mainWindow->getTimeline(),
        m_dataManager, m_algorithmController);
    connect(m_videoPlayerController,
            &VideoPlayerController::sig_hasStatusMessage, m_mainWindow,
            &MainWindow::slot_displayStatusMessage);
    connect(m_mainWindow, &MainWindow::sig_deleteAllKeyframes,
            m_videoPlayerController,
            &VideoPlayerController::slot_deleteAllKeyframes);
    connect(m_mainWindow, &MainWindow::sig_deleteKeyframesBoundaries,
            m_videoPlayerController,
            &VideoPlayerController::slot_deleteKeyframes);
    connect(m_mainWindow, &MainWindow::sig_resetBoundaries,
            m_videoPlayerController,
            &VideoPlayerController::slot_resetBoundaries);

    connect(m_algorithmController, &AlgorithmController::sig_stopPlay,
            m_videoPlayerController, &VideoPlayerController::slot_stopPlay);

    // ExportController manages algorithm used widget and reconstruct widget and
    // delegates export of images and 3d-reconstruction
    m_exportController = new ExportController(m_mainWindow->getOutputWidget(),
                                              m_dataManager, m_colmapWrapper);

    connect(m_exportController, &ExportController::sig_hasStatusMessage,
            m_mainWindow, &MainWindow::slot_displayStatusMessage);
    connect(m_exportController, &ExportController::sig_stopPlay,
            m_videoPlayerController, &VideoPlayerController::slot_stopPlay);
    connect(m_exportController, &ExportController::sig_exportStarted, this,
            &Controller::slot_exportStarted);
    connect(m_exportController, &ExportController::sig_exportFinished, this,
            &Controller::slot_exportFinished);
    connect(m_exportController, &ExportController::sig_exportAborted, this,
            &Controller::slot_exportFinished);
    connect(m_videoPlayerController, &VideoPlayerController::sig_read,
            m_exportController, &ExportController::slot_nextImageOnPlayer);

    // AutoExecutor is used for the automatic Execution
    m_automaticController->setExporController(m_exportController);
    connect(m_automaticController->autoExec(), &AutomaticExecutor::sig_stopPlay,
            m_videoPlayerController, &VideoPlayerController::slot_stopPlay);
    connect(m_automaticController->autoExec(),
            &AutomaticExecutor::sig_hasStatusMessage, m_mainWindow,
            &MainWindow::slot_displayStatusMessage);

    setInputWidgetInfo();  // initialize input widget with information about new
                           // input data
    ...
    connect(m_dataManager->getHistory(), &History::sig_historyChanged, this,
            &Controller::slot_historyChanged);

    // Create the StackController
    m_stackController = new StackController(
        m_mainWindow->getOpStack(), m_dataManager->getHistory(),
        m_mainWindow->getSamplingWidget(), m_exportController);
    connect(m_videoPlayerController, &VideoPlayerController::sig_toggleKeyframe,
            m_stackController, &StackController::slot_toggleKeyframe);
    connect(m_videoPlayerController,
            &VideoPlayerController::sig_deleteAllKeyframes, m_stackController,
            &StackController::slot_deleteAllKeyframes);
    connect(m_videoPlayerController,
            &VideoPlayerController::sig_deleteKeyframes, m_stackController,
            &StackController::slot_deleteKeyframes);
    connect(m_algorithmController, &AlgorithmController::sig_algorithmFinished,
            m_stackController, &StackController::slot_algorithmFinished);
    connect(m_algorithmController,
            &AlgorithmController::sig_keyframesChangedByPlugin,
            m_stackController, &StackController::slot_keyframesChangedByPlugin);
    connect(m_exportController, &ExportController::sig_exportFinished,
            m_stackController, &StackController::slot_exportFinished);
    ...
    m_mainWindow->getVideoPlayer()->updateRoi(params->getUseRoi() ? roi
                                                                  : QRect());

// default working resolution: 720p if input is larger
#define DEFAULT_WORKING_RESOLUTION_HEIGHT 720
    if (params->getOriginalResolution().getHeight() >
        DEFAULT_WORKING_RESOLUTION_HEIGHT) {
        float aspect_ratio =
            static_cast<float>(params->getOriginalResolution().getWidth()) /
            params->getOriginalResolution().getHeight();
        m_mainWindow->getSamplingWidget()->setResolution(
            Resolution(static_cast<int>(DEFAULT_WORKING_RESOLUTION_HEIGHT *
                                        aspect_ratio),
                       DEFAULT_WORKING_RESOLUTION_HEIGHT)
                .toString());
    }
