 ...
 *
 * @brief The GeoMapPlugin allows the user to select the images, which are to be
 * used for the 3D reconstruction, based on their geographic position.
 *
 * In this, the location (derived from the meta data) of the images are
 * displayed on a map. The user is then prompted to draw a region of interest in
 * the form of a polygon. All images whose geographic location lie inside this
 * polygon are than exported and prepared for the 3D reconstruction.
 *
...
class GeoMap : public IAlgorithm {
   ...
   public slots:
    ...
    /**
     * @brief onGpsSelected Slot is triggered when a new user selcted polyogn is
     * created
