...
//==================================================================================================
GeoMap::~GeoMap() {
    if (mpQuickViewContainerWidget) mpQuickViewContainerWidget->deleteLater();
}
...
std::vector<uint> GeoMap::sampleImages(const std::vector<uint>& imageList,
    ...
    // intersect given images (selection from previous algorithms) and images
    // selected on map
    auto end = std::set_intersection(imageList.begin(), imageList.end(),
                                     frames_inside_polygon.begin(),
                                     frames_inside_polygon.end(), ret.begin());
...
//==================================================================================================
void GeoMap::initialize(iReader* reader, QMap<QString, QVariant> buffer,
                        signalObject* sigObj) {
    ...
    connect(mpSigObj, SIGNAL(sig_keyframesChanged(std::vector<uint>)), this,
            SLOT(onKeyframesChanged(std::vector<uint>)));

    //--- Get reader of metadata. if reader is not available, return
    MetaData* metaDataReader = mpReader->getMetaData();
    if (metaDataReader == nullptr) return;
...
void GeoMap::onNewMetaData() {
    ...
    if (mpQuickViewContainerWidget) reinitializeQmlMap();
}
...
void GeoMap::onKeyframesChanged(std::vector<uint> keyframes) {
    ...
    if (mpQuickViewContainerWidget && mpMapWidget && mpMapWidget->isVisible()) {
        // updating individual qml items is expensive, so only do it if there
        // are few items to update! The boundary is heavily dependent on
        // hardware, i.e.
        // - on low-end laptop, for 100+ points, it is faster to redraw from
        // scratch
        // - on high-end desktop, 100k points to update is still similar speed
        // to redrawing
    ...
    /*
        ...
        if (mpQuickViewContainerWidget && mpMapWidget &&
       mpMapWidget->isVisible())
        {
            reinitializeQmlMap();
        }*/
}
...
void GeoMap::createSettingsWidget(QWidget* parent) {
    ...
    pHLayout->addSpacerItem(new QSpacerItem(
    ...
    for (QString metaName : metaReader) {
        if (metaName.startsWith("GPS")) {
            ...
            for (QVariant var : mMetaData) {
                mGpsData.append(
                    QPair<QPointF, bool>(gpsHashToLatLong(var), true));
            }
...
std::vector<unsigned int> GeoMap::getFramesInsidePolygon() {
    ...
    for (int i = 0; i < mGpsData.size(); i++) {
        // Check for every gps point, if it's inside the polygon and set the
        // mPointInsidePolygon list accordingly
...
QPointF GeoMap::gpsHashToLatLong(QVariant hash) {
    ...
    double latitude = (gpsHash.find("GPSLatitudeRef").value().toString() == "N")
                          ...
                          : latitude_abs * -1;
    double longitude =
        (gpsHash.find("GPSLongitudeRef").value().toString() == "E")
            ? longitude_abs
            : longitude_abs * -1;
...
void GeoMap::initializeQmlMap() {
    ...
    mpQuickViewContainerWidget =
        QWidget::createWindowContainer(pQuickView, mpMapWidget);
    mpMapHandler.reset(new MapHandler());
    pQuickView->engine()->clearComponentCache();
    pQuickView->rootContext()->setContextProperty("handler",
                                                  mpMapHandler.get());
    ...
    if (!pQuickView->errors().isEmpty()) {
        ...
        QObject::connect(qmlRoot, SIGNAL(deleteSelection()), mpMapHandler.get(),
                         SLOT(onQmlDeleteSelection()));
        QObject::connect(qmlRoot, SIGNAL(selectionBack()), mpMapHandler.get(),
                         SLOT(onQmlSelectionBack()));
        QObject::connect(qmlRoot, SIGNAL(selectionForward()),
                         mpMapHandler.get(), SLOT(onQmlSelectionForward()));

        QObject::connect(mpMapHandler.get(), &MapHandler::gpsClicked, this,
                         &GeoMap::onGpsClicked);
        QObject::connect(mpMapHandler.get(), &MapHandler::gpsSelected, this,
                         &GeoMap::onGpsSelected);

        //--- set default map center to IOSB
        mpMapHandler->emitAdjustMapCenter(
...
void GeoMap::reinitializeQmlMap() {
    ...
    if (mpQuickViewContainerWidget) delete mpQuickViewContainerWidget;
...
double GeoMap::distanceBetweenPoints(int first, int second) {
    ...
    QPointF secondLatLong =
        QPointF(secondGPS.latitude(), secondGPS.longitude());
