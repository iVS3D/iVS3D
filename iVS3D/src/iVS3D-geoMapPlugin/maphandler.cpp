...
void MapHandler::newMapItems()
{
    ...
    // If there are only two points yet, don't calculate points inside the QGeoPath
    if (path.size() <= 2)
    {
        return;
    }
    ...
    // Update gps Map to updates points on the map
    QMapIterator<QPointF, bool> mapIt(mGpsMap);
    while (mapIt.hasNext())
    {
        mapIt.next();
        QPointF point = mapIt.key();
        if (polF.containsPoint(point, Qt::OddEvenFill))
        {
            // Point is inside the polygon
            if (mGpsMap[point] == false)
            {
                mGpsMap[point] = true;
                // Save which points have changed
                mChangedPoints.insert(point);
            }
        }
        else
        {
            // Point is outside the polygon
            if (mGpsMap[point] == true)
            {
                mGpsMap[point] = false;
                mChangedPoints.insert(point);
            }
    ...
    // Update the changed points
    for (QPointF point : mChangedPoints)
    {
        emitSetPoint(mMapItems[point], mGpsMap[point]);
    }
