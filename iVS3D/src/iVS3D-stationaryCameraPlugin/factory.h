 ...
 *
 * @brief The Factory class is responsible for creating and destroying as well
 * as managing a single object of the ImageGatherer and FlowCalculator class. It
 * creates the hardware specified type of each of these classes. (CPU or CUDA)
 *
 * @author Dominic Zahn
 *
 * @date 2022/4/5
...
class Factory {
   public:
    ...
    /**
     * @brief createComponents creates all neccessary components for the sample
     * process (ImageGatherer, FlowCalculator, KeyframeSelector)
     * @param futureFrames as list of all frames that will be requested from the
     * imageGatherer in the future
     * @param reader is the object which the imageGatherer uses to extract the
     * images
     * @param useCuda defines the implementation which should be used (CPU or
     * CUDA)
     * @param selectorName name of the selected KeyframeSelector
     * @param selectorSettings parameters from ui, which will be used to select
     * Keyframes
