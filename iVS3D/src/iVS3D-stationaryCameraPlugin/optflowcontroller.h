...
#define SELECTOR_DESCRIPTION                                              \
    QObject::tr(                                                          \
        "Removes all frames where the camera is stationary. A frame is "  \
        "declared stationary if its camera movement is lower than given " \
        "percentage of the median of all camera movements.")
...
#define DESCRIPTION_DOWNSAMPLE                                               \
    QObject::tr(                                                             \
        "If enabled a resolution of 720p will be used for the algorithm to " \
        "speed up computation. This will hurt the accuracy of the result "   \
        "slightly. It however won't change the export resolution. This "     \
        "parameter will be disabled if the input resolution is lower or "    \
        "equal than 720p.")
...
class SmoothController : public IAlgorithm {
   ...
   public:
     ...
     *      <QVariant> samplingResolution (enable/disable diffrent resolution
     * for sampling) <QVariant> selector_threshold
     *  </QMap>
