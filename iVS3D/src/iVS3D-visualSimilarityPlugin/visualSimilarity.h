 ...
 * @ingroup Plugin
 *
 * @brief This plugin computes a feature vector for each frame using a given
 * neural network. These identiviers are clustered with k-Means under the cosine
 * similarity metric. It therefore selects k keyframes based on there visual
 * similarity.
 */
...
#define UI_FRAMEREDUCTION_DESC \
    QObject::tr("Reduces the amount of selected frames by the factor K.")
#define UI_NNNAME_NAME QObject::tr("Selected Neural Network")
#define UI_NNNAME_DESC                                                      \
    QObject::tr(                                                            \
        "The drop-down shows all files in the folder "                      \
        "plugins/ressources/neural_network_models/ that follow the format " \
        "ImageEmbedding_NAME_DIMENSION_WIDHTxHEIGHT.onnx")
#define UI_NNNAME_BT_DESC \
    QObject::tr("Resets the drop-Down and reloads available neural networks.")
...
class VisualSimilarity : public IAlgorithm {
   ...
   public:
    ...
    /**
     * @brief sampleImages Create an keyframe list with indices of the
     * keyframes. The algorithm reports progress to the Progressable *receiver
     * by calling Progressable::slot_makeProgress. If the *stopped bool is set
     * to @a true the algorithm should abort the calculations and exit.
     *
     * @param imageList is a preselection of frames
     * @param receiver is a progressable, which displays the progress made so
     * far
    ...
    /**
     * @brief initialize is called after new images have been loaded. the plugin
     * parameters can be selected based on the images.
     * @param reader for the images
     * @param buffer contains persistently stored plugin data from the project
     * file. This includes data stored using getSettings() and
     * generateSettings()
   ...
   private:
    ...
    const QRegularExpression m_nnNameFormat =
        QRegularExpression("^(ImageEmbedding)\\w+.onnx$");
