...
std::vector<uint> VisualSimilarity::sampleImages(
    const std::vector<unsigned int>& imageList, Progressable* receiver,
    ...
    // Read nn
    if (!QFile::exists(QCoreApplication::applicationDirPath() + RESSOURCE_PATH +
                       m_nnFileName)) {
    ...
    if (!m_neuralNet || m_neuralNet->gpuId() != (useCuda ? 0 : -1)) {
        QString neuralNetAbsolutePath = QCoreApplication::applicationDirPath() +
                                        RESSOURCE_PATH + m_nnFileName;
        auto createResult = NN::NeuralNetFactory::create(
            neuralNetAbsolutePath.toStdString(), useCuda);
        if (!createResult) {
            displayErrorMessage(
    ...
    if (m_neuralNet->inputShape().size() != 4) {
        ...
        displayErrorMessage(
            tr("The selected neural network does not have the expected input "
               "shape. Please select a different neural network."));
        m_neuralNet = nullptr;
        return imageList;
    }
    ...
    if (height == -1 || width == -1) {
        auto image = m_reader->getImage(imageList[0]);
        if (height == -1) height = image.rows;
        if (width == -1) width = image.cols;
    }
    std::cout << "Neural Network Input Shape: "
              << NN::shapeToString(m_neuralNet->inputShape()) << std::endl;
    ...
    // check the output shape of the neural network, we expect NxF where F is
    // the feature dimension
    if (m_neuralNet->outputShape().size() != 2) {
        auto m_neuralNetShape = m_neuralNet->outputShape();
        SHAPE_DEBUG_PRINT(m_neuralNetShape);
        displayErrorMessage(
            tr("The selected neural network does not have the expected output "
               "shape. Please select a different neural network."));
        m_neuralNet = nullptr;
        return imageList;
    }
    ...
    if (batch == -1 && useCuda) {
        ...
        batchSize =
            floor(availableMemory / (width * height * 3 * 32) *
                  MEM_THRESEHOLD);  // RAM/(W*H*C*32)*thresehold <- CV_32F
        if (batchSize > MAX_BATCH) batchSize = MAX_BATCH;

        std::cout << "Detected "
                  << std::round(availableMemory / 10000000.0) / 100.0
                  << "GB free memory." << std::endl;

        if (batchSize < 1) {
            batchSize = 1;
            std::cerr << "Warning: Not enough memory available! Trying anyway "
                         "with batch size 1..."
                      << std::endl;
        }
    ...
    for (int i = 0; i < frameCount; i += batchSize) {
        ...
        std::cout << "Processing batch from " << i << " to "
                  << std::min(i + batchSize, frameCount) << std::endl;
        ...
        if (fullBatchAvailableInBuffer) {
            ...
            if (totalFeatureVector.empty())
                ...
                cv::vconcat(totalFeatureVector, bufferedBatch,
                            totalFeatureVector);
        }
        ...
        if (!fullBatchAvailableInBuffer) {
            ...
            for (uint j = i; j < i + batchSize && j < frameCount; j++) {
                int progress = 100.0 * (imageList[j] - *imageList.begin()) /
                               (*(imageList.end() - 1) - *imageList.begin());
                QString progressDesc =
                    tr("Calculating feature vector for frame ") +
                    QString::number(imageList[j]);
                displayProgress(receiver, progress, progressDesc);

                cv::Mat img = m_reader->getImage(imageList[j]);
                imgVec.push_back(img);
            }

            // lambda expression to feed the neural network
            auto feedImages = [this, nnInputShape, &inferenceError](
                ...
                auto result =
                    Tensor::fromCvMats(imgs, nnInputShape, 1.0 / 255.0, NN_MEAN,
                                       NN_STD)
            ...
            if (inferenceError.has_value()) {
                if (inferenceError.value().code() ==
                    NN::ErrorCode::OutOfMemory) {
                    if (batchSize <= 1) {
                        displayErrorMessage(
                            tr("Not enough memory available to feed the neural "
                               "network!\n To resolve this you can:\n- Reduce "
                               "the working resolution or\n- Select a smaller "
                               "neural network or\n-Disable CUDA"));
                        futureFeedNN.cancel();
                        m_neuralNet = nullptr;
                        return imageList;
                    ...
                    batchSize =
                        std::max(1, batchSize / 2);  // reduce batch size
                    i -= batchSize;
                    std::cerr << "Not enough memory available to feed the "
                                 "neural network. Reducing the batch size to "
                              << batchSize << std::endl;

                    continue;
                } else {
                    // Error we cant handle here, display to the user and end
                    // the sampling
                    displayErrorMessage(
                        tr("An error occurred while feeding the neural "
                           "network:\n%1")
            ...
            futureFeedNN =
                QtConcurrent::run(feedImages, imgVec, &totalFeatureVector);
        }
        // --------------------------------------------------------------
    }
    ...
    cv::Mat out;
    cv::normalize(getFeatureVector(totalFeatureVector, 0),
                  normalizedTotalFeatureVector, 1.0, 0.0, cv::NORM_MINMAX);
    for (int i = 1; i < totalFeatureVector.rows; i++) {
        cv::normalize(getFeatureVector(totalFeatureVector, i), out, 1.0, 0.0,
                      cv::NORM_MINMAX);
        if (out.empty()) {
            displayErrorMessage(
                tr("One of the resulting feature vectors is empty. This maybe "
                   "caused by a not suitable neural network."));
            return imageList;
        }
    ...
    cv::kmeans(normalizedTotalFeatureVector, targetFrames, labels,
               cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::COUNT,
                                10, 1.0),
               1, cv::KmeansFlags::KMEANS_PP_CENTERS, centers);
    ...
    for (int i = 0; i < frameCount; i++) {
        ...
        cv::Mat currFeatureVector =
            getFeatureVector(normalizedTotalFeatureVector, i);

        if (currCenter.empty() || currFeatureVector.empty()) {
            displayErrorMessage(
                tr("Wrong feature vector dimension for selected neural "
                   "network. (this dimension was specified in the name of the "
                   "file, which contains the neural network)"));
            return imageList;
        }
    ...
    for (int i = 0; i < frameCount; i++) {
        ...
        if (i < frameCount - 1) ss << ",";
    }
    logFile->addCustomEntry("labels", QString::fromStdString(ss.str()),
                            "Additional Info");
    //
...
void VisualSimilarity::initialize(iReader* reader,
    ...
    double fps = reader->getAvgFps();
    m_frameReduction = fps > 0.0 ? (int)fps : 30;
    m_frameReduction =
        m_frameReduction < reader->getImageCount() ? m_frameReduction : 2;
...
void VisualSimilarity::setSettings(QMap<QString, QVariant> settings) {
    // frame reduction
    QMap<QString, QVariant>::iterator itFrameReduction =
        settings.find(FRAMEREDUCTION_JSON_NAME);
    ...
    // nn name
    QMap<QString, QVariant>::iterator itNNname =
        settings.find(NNNAME_JSON_NAME);
    if (itNNname != settings.end()) {
        m_nnFileName = itNNname.value().toString();
    }
}

QMap<QString, QVariant> VisualSimilarity::generateSettings(
    Progressable* receiver, bool useCuda, volatile bool* stopped) {
    // this routine runs if the user activly wants to generate optimal settings
    // for the given images. It can be slow or heavy computation here, the user
    // is prompted with an progress bar.
    (void)receiver;
    (void)useCuda;
    (void)stopped;
...
void VisualSimilarity::slot_reloadNN(int index) {
    QStringList nnNameList =
        collect_nns(QCoreApplication::applicationDirPath() + RESSOURCE_PATH);
    m_nnNameInput->clear();
    m_nnNameInput->addItems(nnNameList);
}

void VisualSimilarity::displayProgress(Progressable* p, int progress,
                                       QString msg) {
    QMetaObject::invokeMethod(p, "slot_makeProgress", Qt::DirectConnection,
                              Q_ARG(int, progress), Q_ARG(QString, msg));
}

void VisualSimilarity::displayMessage(Progressable* p, QString msg) {
    QMetaObject::invokeMethod(p, "slot_displayMessage", Qt::DirectConnection,
                              Q_ARG(QString, msg));
}
...
void VisualSimilarity::sendBuffer(cv::Mat bufferMat,
                                  std::vector<uint> calculatedIdx) {
    ...
    for (int y = 0; y < size.height; y++) {
        for (int x = 0; x < size.width; x++) {
            ...
            if (x < size.width - 1) matStream << BUFFER_FEATURE_DELIMITER_X;
        }
        if (y < size.height - 1) matStream << BUFFER_FEATURE_DELIMITER_Y;
    }
...
void VisualSimilarity::createSettingsWidget(QWidget* parent) {
    ...
    // find all nns available for deep visual similarity in
    // resources/neural_network_models
    auto availableNNs =
        collect_nns(QCoreApplication::applicationDirPath() + RESSOURCE_PATH);
    ...
    if (availableNNs.isEmpty()) {
        ...
        QString errorMessage =
            tr("<div style='border: 2px solid red; padding: 10px;'>"
               ...
               "<code>%4</code><br><br>"
               "<p>%5 <a "
               "href='https://github.com/iVS3D/iVS3D-models'>%6</a>.</p>")
                .arg(tr("ERROR:"),
                     tr("No neural network models for deep visual similarity "
                        "were found!"),
                     tr("Please add your models to the plugin resources "
                        "directory and restart iVS3D:"),
                     QCoreApplication::applicationDirPath() + RESSOURCE_PATH,
                     tr("You can find our neural network models"), tr("here"));
        ...
        this->m_settingsWidget->setSizePolicy(QSizePolicy::Minimum,
                                              QSizePolicy::Minimum);
    ...
    frameReductionWidget->layout()->addWidget(
        new QLabel(UI_FRAMEREDUCTION_NAME));
    ...
    m_frameReductionInput->setMaximum(m_reader->getImageCount());
    ...
    connect(m_frameReductionInput, QOverload<int>::of(&QSpinBox::valueChanged),
            this, [=](int v) { m_frameReduction = v; });
    // nn_name input
    ...
    //    connect(m_nnNameInput, QOverload<int>::of(&QComboBox::activated),
    //    this, &VisualSimilarity::slot_reloadNN);
    connect(m_nnNameInput, &QComboBox::currentTextChanged, this,
