...
{
    // Use caller flag (no runtime detection)
    g_useCuda = useCuda;
    ...
    if (imageList.size() == imageList.back() - imageList[0] + 1) {
        if (m_blurValues[0] == 0) {
            m_blurValues = m_usedBlur->calcFullBluriness(m_reader,
                                                         receiver,
                                                         stopped,
                                                         imageList[0],
                                                         imageList.back(),
...
void Blur::initialize(Reader *reader, QMap<QString, QVariant> buffer, signalObject *sig_obj)
{
    ...
    if (m_settingsWidget) {
        if (m_buffer.contains(m_usedBlur->getName())) {
            ...
            QString info = currentBlurValue == 0 ? tr("not calculated")
                                                 : QString::number(currentBlurValue);
            m_infoLabel->setText(tr("Blur value for the current image is ") + info);
        } else {
            m_infoLabel->setText(tr("Blur value for the current image is not calculated"));
        }
    }
    if (m_sigObj)
        connect(m_sigObj,
                SIGNAL(sig_selectedImageIndex(uint)),
                this,
...
void Blur::createSettingsWidget(QWidget *parent)
{
    ...
    QLabel *windowSize = new QLabel(tr("Number of images used on each side for the local window"));
    ...
    QLabel *localDeviation = new QLabel(
        tr("Keep images whose sharpness is at least this percent of the local median."));
    ...
    m_infoLabel = new QLabel(tr("Blur value for the current image is not calculated"));
...
// Percent-of-median local threshold across full range [start, end]
std::vector<uint> Blur::sampleAllImages(
    Reader *reader, Progressable *receiver, volatile bool *stopped, int start, int end)
{
    std::vector<unsigned int> keptImages;
    ...
    for (int i = start; i <= end; i++) {
        ...
        int ws = m_windowSize;
        int wStart = std::max(start, i - ws);
        int wEnd = std::min(end, i + ws);
        ...
        if (m_blurValues[i] == 0) {
            m_blurValues[i] = m_usedBlur->calcOneBluriness(reader, i);
        }

        double med = medianOf(windowVals);
        if (med <= 1e-9) {
            keptImages.push_back(i);
            continue;
        }

        double ratio = m_localDeviation / 100.0;
        if (m_blurValues[i] >= med * ratio) {
            keptImages.push_back(i);
        }
    }
    return keptImages;
...
{
    return tr("Calculate blur value for image ") + QString::number(curr) + tr(" of ")
