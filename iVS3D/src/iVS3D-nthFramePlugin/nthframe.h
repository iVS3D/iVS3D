 ...
 *
 * @brief The NthFrame class implements the IAlgorithm plugin interface and
 * provides functionality needed to select every Nth frame as keyframe. The
 * class also provides methods for changing N and visualizing the plugin in the
 * core application using the name and settings.
 *
 * @author Dominik Wüst
 *
 * @date 2021/02/14
...
class NthFrame : public IAlgorithm {
   ...
   public:
    ...
    /**
     * @brief showSettings Show a SettingsDialog to the user. This allows to
     * change the value of N.
     * @param parent The parent widget for the SettingsDialog. This is needed to
     * determin style and visibility.
     */
    QWidget* getSettingsWidget(QWidget* parent);

    /**
     * @brief sampleImages Create an keyframe list with indices to every nth
     * sharp image. Sharp images are specified by index in sharpImages list. The
     * algorithm reports progress to the Progressable *receiver by calling
     * Progressable::slot_makeProgress. Setting the *stopped bool to @a true
     * exits the algorithm.
     *
