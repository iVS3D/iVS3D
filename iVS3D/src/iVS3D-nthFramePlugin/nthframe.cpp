...
std::vector<uint> NthFrame::sampleImages(
    ...
    volatile bool* stopped, bool, LogFileParent* logFile) {
    logFile->startTimer(LF_TOTAL);
    int expectedKfDistance =
        roundf((float)m_N / ((float)imageList.size() / m_numFrames));
    ...
    for (int i = 0; i < (int)imageList.size(); i++) {
        ...
        if (ctr >= m_N || (m_keepLonely &&
                           (imageList[i] - lastKfIndex > expectedKfDistance))) {
...
QString NthFrame::getName() const { return tr("Nth image selection"); }

void NthFrame::initialize(iReader* reader, QMap<QString, QVariant>,
                          signalObject* so) {
    m_reader = reader;
    m_numFrames = reader->getImageCount();
    if (reader->getAvgFps() == -1) {
        m_fps = 30;
    } else {
        m_fps = round(reader->getAvgFps());
    }

    m_N = m_fps;
    if (m_settingsWidget) m_spinBox->setValue(m_N);
}
...
void NthFrame::slot_nChanged(int n) { m_N = (unsigned int)n; }

void NthFrame::slot_checkboxToggled(bool checked) { m_keepLonely = checked; }
